...
def cuda_home(version=None):
    ...
    try:
        ...
        pass
    # Guess #3: common locations
    if is_windows():
        path = 'C:/Program Files/NVIDIA GPU Computing Toolkit/CUDA/v*.*'
    else:
        path = '/usr/local/cuda*'
    homes = glob.glob(path)
    for home in homes:
        if version_ok(home, version):
            print(f'-- Found CUDA {v2str(cuda_version(home))}.')
            return home
