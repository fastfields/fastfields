#include "impl/precond_grid_common.h"
#include "checks.h"
#include <ATen/ATen.h>
#include <vector>
#include <utility>
...
Tensor precond_grid(
  ...
  if (gradient.is_cuda())
    return cuda::precond_grid_impl(hessian, gradient, solution, weight,
        absolute, membrane, bending, lame_shear, lame_div,
        ArrayRef<double>(voxel_size), BoundVectorRef(bound));
  else
    return cpu::precond_grid_impl(hessian, gradient, solution, weight,
        absolute, membrane, bending, lame_shear, lame_div,
        ArrayRef<double>(voxel_size), BoundVectorRef(bound));
}
