...
#define FF_CHECK_NOT_EMPTY(value)                                      \
  for (int64_t i = 2; i < value.dim(); i++) {                          \
    TORCH_CHECK(value.size(i) > 0,                                     \
      ...
      " being empty"); }
#define FF_CHECK_GRID_TARGET_COMPAT(value1, value2)                    \
    TORCH_CHECK(                                                       \
    ...
    value1.sizes());
#define FF_CHECK_VEC_LENGTH(value, dim)                                \
  TORCH_CHECK(                                                         \
    ((int64_t)(value.size()) == dim - 2),                              \
    "(): expected ", dim, #value " elements but got ", value.size());
#define FF_CHECK_VEC_NOT_EMPTY(value)                                  \
